# - [X] Check SNPs and assign marker ID
# - [X] Check if reads & mates cover all SNP positions
# - [X] Construct haplotypes based on SNP combinations
# - [ ] Collate / sort & index depending on situtation --> if vcf, sort by coord, else collate
# - [ ] if vcf is included --> check monoclonal
# - [ ] if non-monoclonal, attempt to group reads together
...
def init_argparser():
    ...
    #     "--vcf",
    #     default=None,
    #     help="Input VCF file (for WGS, filtered only for counts, no indels)",
    # )
    p.add_argument("--sortby", choices=["coord", "collate"], default="collate", help="BAM sorting method, default = collate")
    p.add_argument("-o", "--outfile", required = True, help="Output filename")
    p.add_argument("infile", help="Input BAM file (sorted)")
    return p
...
def construct_pseudo_haplotypes(variants, alignments):
    ...
    # process_from_vcf = vcf is not None
...
def check_monoclonal(vcf, variants):
    # monoclonal at the block (if homozygous at the block)
    ...
...
def main(args):
    ...
    #     try:
    #         vcf.subset_samples([args.sample])
    #     except ValueError:
    #         cexit(f"Sample {args.sample} not found in VCF file {args.vcf}")
